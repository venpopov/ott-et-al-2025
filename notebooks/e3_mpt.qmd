...
bmmformula <- bmf(
  A ~ tree1*cR + tree3*cF + b + b2 + DR*(tree1+tree2) + DF*(tree3+tree4) + a,
  B ~ tree2*cR + tree4*cF + b + b2 + DR*(tree1+tree2) + DF*(tree3+tree4),
  N ~ b + DN*tree5,
  cR ~ 1,
  cF ~ 1,
  b2 ~ 1,
...
bmmodel <- m3(
  ...
  choice_rule = "softmax", 
  version = "custom",
  links = list(cR = "log", cF = "log", b2 = "log", DR = "log", DF = "log", DN = "log", a = "log")
)
